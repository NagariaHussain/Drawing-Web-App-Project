...
function Toolbox() {
	...
	var addToolIcon = function(icon, name) {
		...
		// Creating a title using the name
		let toolTitle = generateTitleString(name);

		sideBarItem.attribute('title', toolTitle);
		sideBarItem.mouseClicked(toolbarItemClick);
	};

	// Function to generate title using tool name
	function generateTitleString(toolName) {
		let title = "";

		// Splitting on Capital letters
		const nameTokens = toolName.split(/(?=[A-Z])/);

		// Joining with spaces
		for (token of nameTokens) {
			title += (token + " ");
		}
